...
class MIDIMatrixManager {
    ...
    int register_source(char *handle) {
        strcpy(sources[sources_count].handle, handle);
        return sources_count++;
    }
    int get_source_id_for_handle(char *handle) {
    ...
    void connect_source_target(char *source_handle, char *target_handle) {
        this->connect_source_target(
            this->get_source_id_for_handle(source_handle),
            this->get_target_id_for_handle(target_handle)
        );
    }
    void disconnect_source_target(char *source_handle, char *target_handle) {
    ...
    int register_target(MIDIOutputWrapper *target) {
        return this->register_target(target->label, target);
    }
    int register_target(const char *handle, MIDIOutputWrapper *target) {
        strcpy(targets[targets_count].handle, handle);
        targets[targets_count].wrapper = target;
        return targets_count++;
    }

    int get_target_id_for_handle(char *handle) {
